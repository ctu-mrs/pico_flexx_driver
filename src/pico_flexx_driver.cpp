...
/*//{ constructor */
  PicoFlexx(const ros::NodeHandle &nh = ros::NodeHandle(), const ros::NodeHandle &priv_nh = ros::NodeHandle("~"))
      ...
      : royale::IDepthDataListener(),
        royale::IExposureListener(),
        nh(nh),
        priv_nh(priv_nh),
        server(lockServer, priv_nh)
#else
      : royale::IDepthDataListener(),
        royale::IExposureListener2(),
        nh(nh),
        priv_nh(priv_nh),
        server(lockServer, priv_nh)
#endif
...
/*//{ initialize() */
  bool initialize() {
    ...
    if (!selectCamera(sensor) || !setUseCase((size_t)useCase) || !setExposureModeAllStreams(automaticExposure, automaticExposureStream2) ||
        !getCameraSettings(params) || !createCameraInfo(params)) {
      return false;
    }
...
/*//{ getCameraSettings() */
  bool getCameraSettings(royale::LensParameters &params) {
    ...
    if (cameraDevice->getLensParameters(params) == royale::CameraStatus::SUCCESS) {
      ...
      OUT_INFO("fx: " FG_CYAN << params.focalLength.first << NO_COLOR ", fy: " FG_CYAN << params.focalLength.second << NO_COLOR ", cx: " FG_CYAN
                              << params.principalPoint.first << NO_COLOR ", cy: " FG_CYAN << params.principalPoint.second << NO_COLOR);
      if (params.distortionRadial.size() == 3) {
        OUT_INFO("k1: " FG_CYAN << params.distortionRadial[0] << NO_COLOR ", k2: " FG_CYAN << params.distortionRadial[1] << NO_COLOR ", p1: " FG_CYAN
                                << params.distortionTangential.first << NO_COLOR ", p2: " FG_CYAN << params.distortionTangential.second
...
/*//{ extractData() */
  void extractData(const royale::DepthData &data, sensor_msgs::CameraInfoPtr &msgCameraInfo, sensor_msgs::PointCloud2Ptr &msgCloud,
    ...
    if (!(status[streamIndex][MONO_8] || status[streamIndex][MONO_16] || status[streamIndex][DEPTH] || status[streamIndex][NOISE] ||
